...
void SupprimerChar(char m[],int Taille ,int pos ){
     ...
     **/
        ...
        int i;
        for (i=pos;i<(Taille-1);i++) m[i]=m[i+1];
        m[Taille-1]=0;
...
void AjoutezChar(char m[],int Taille ,int pos ,char c){
    ...
    int i;
    for (i=Taille-1;i>=pos;i--) m[i+1]=m[i] ;
    m[pos]=c;
    m[Taille+1]=0;
...
int DiffiranceEntre2Date(int jour1,int mois1 ,int annee1,int jour2,int mois2,int annee2){

        /** CALCULE LE NOMBRE DE JOUR QUE DATE1 ARRIVE A DATE2
         ** C'EST LA DATE1 DEJA DEPASSER DATE2 ON RETURN 0
         **/

         if (annee1>annee2) return 0;
         if ((annee1==annee2)&&(mois1>mois2)) return 0;
         if ((annee1==annee2)&&(mois1==mois2)&&(jour1>=jour2)) return 0;
         int i=0;
         do{
            i++;
...
int strstrcase(char* ch,char* rh){
    ...
    int i=0,j=0;
    while(ch[i]!=0 && rh[j]!=0){
        if (tolower(ch[i])==tolower(rh[j])) j++;
            else {
                j=0;
            if (tolower(ch[i])==tolower(rh[j])) j++;
            }
