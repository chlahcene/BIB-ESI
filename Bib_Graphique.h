...
void cputcxyBlanc(int x,int y,char ch[],int T,int pos[]){
    ...
    /** affichez les cractère ch[i] de chain un par un  à la postion (x+pos[i],y) dans ecran
     * affichez seulement T cractére
     * si la longueur de ch < T donc terminera le affichage par de blanc
     **/
     ...
     for (i=0;i<T;i++){
        if (i<m) cputsxyBlanc(x+pos[i],y,&(ch[i]),1);
         else cputsxyBlanc(x+pos[i],y," ",1); // on complete la rest par de blanc
   }
}
void AffichezButon(char* ch,int x,int y,int l,int color){
     ...
     **/
     ...
     for (i=0;i<3;i++){
        ...
        cputsxyBlanc(x-1,y+i-1," ",l); /// affichez un text avec seulement de blanc
        }
     i=strlen(ch);
...
void AffichezTexte(char text[],int x,int y,int Longueur ,int color,int ColorBackground){
     ...
     **/
     ...
     do{
        ...
        if ((f%Longueur+j)>=Longueur) gotoxy(x,wherey()+1);
            s[j]=0;
            printf("%s",s);
     textbackground(colorbackground);
     textcolor(colorText);
...
void Afficher_Emprunteur (maillon_Emprunteur* P,int x,int y,int Longueur ,int Hauteur,int color){
    ...
    sprintf(m,"%d/%d/%d",P->dateDeInscription.days,P->dateDeInscription.month,P->dateDeInscription.years);
    ...
    if (P->Emprunter && P->Emprunt!=NULL) {
            ...
            sprintf(m,"%d/%d/%d",P->Emprunt->dateDeRetour.days,P->Emprunt->dateDeRetour.month,P->Emprunt->dateDeRetour.years);
    ...
    if (P->penaliter==0) sprintf(m,"Non");
        else sprintf(m,"IL VA RESTE %d JOURS",P->penaliter);
    cputsxych(x+i+30,y+2+10*pas,m);
    getch();
}
void Afficher_Livre (maillon_Livre* P,int x,int y,int Longueur ,int Hauteur,int color){
    ...
    sprintf(m,"%d/%d/%d",P->dateEdition.days,P->dateEdition.month,P->dateEdition.years);
    ...
    if (P->disponible) {
            ...
            sprintf(m,"%d %s %s",P->Emprunt->Emprunteur->ordreInscription,P->Emprunt->Emprunteur->nom,P->Emprunt->Emprunteur->prenom);
            cputsxych(x+i+30,y+2+7*pas,m);
            sprintf(m,"%d/%d/%d",P->Emprunt->dateDeRetour.days,P->Emprunt->dateDeRetour.month,P->Emprunt->dateDeRetour.years);
            cputsxych(x+i+30,y+2+8*pas,m);
        }
    ...
    cputsxych(x+i+30,y+2+9*pas,m);
    AffichezTexte(P->resume,x+i+30,y+2+10*pas,Longueur-50-i,colorText,colorbackground);
    getch();
    gotoxy(1,1);
    }
void Afficher_Emprunt (maillon_Emprunt* P,int x,int y,int Longueur ,int Hauteur,int color){
    ...
    sprintf(m,"%d/%d/%d",P->dateDeEmprunt.days,P->dateDeEmprunt.month,P->dateDeEmprunt.years);
    cputsxych(x+i+30,y+2+6*pas,m);
    sprintf(m,"%d/%d/%d",P->dateDeRetour.days,P->dateDeRetour.month,P->dateDeRetour.years);
    cputsxych(x+i+30,y+2+7*pas,m);
    if (P->livreRendu) cputsxych(x+i+30,y+2+8*pas,"Oui");
        else cputsxych(x+i+30,y+2+8*pas,"Non");
...
void InscriptionEmprunteur(maillon_Emprunteur**Tete){
   ...
   ** LIRE LES INFORMATION DES ENTRES DES UTILISATAURE ET ET SAUVEGARDEZ DANS LA LISTE
...
debut : AffichezPage(x,y,lg,ht,colorbackground);
...
RST : /// C'EST ICI LE RETOURE DE GOTO C'EST IL AUIYER SUR LE BUTON RESET
    ...
    for (i=0;i<Taille-2;i++){
     ...
     Info[4][8]=0;
     cputcxyBlanc(a,b+4*pas,Info[4],8,&(ps[4][0])); /// ON ECRIT LE CHAINE DE CARACTERE
     int saiser[10]={0};
     saiser[4]=8;
     saiser[5]=2;
     ps[5][0]=0;
     ps[5][1]=20;
     for (j=0;j<2;j++) AffichezButon(buton[j],a+ps[5][j],b+(Taille-1)*pas,10,ColorDeselect);/// AFFICHEZ DEUX BUTTON A L'ECRAN
     int Q=false;
     char c,po[20];
     i=0;j=0;
     do {
        ...
        if ((i==Taille-1)&&(!Q)&&(c==13)){
            ...
            if (j==0) {
                ...
                for (k=0;k<Taille;k++)
                 ...
                 if ((Years<2000)||(Years>2100)) {
                    ...
                    AffichezErreur(x+10,y+1," LA DATE YEARS IL DOIT ENTRE 2000 .. 2100 ");
                    continue;
                 }
                 ...
                 if ((Month<1)||(Month>12)) {
                    ...
                    AffichezErreur(x+10,y+1," LA DATE MONTHE EST INCORECT IL DOIT ENTRE 1 .. 12 ");
                    continue;
                 }
                 ...
                 if ((Days<1)||(Days>k)){
                     sprintf(ya," LA NEMERO DE JOUR EST INCORECT IL DOIT COMPRIT ENTRE 1 .. %d ",k);
        ...
        }else if ((Q&&((c==80)||(c==81)))||(!Q&&(c==13))||(!Q&&(c==9))) {
            ...
            i=(i+1)%Taille; /// ON INCREMANTE I DE FACON NE DEPASSE PAS LE NOMBRE DE UNPUTE A ENTREZ
            if (i==Taille-1) {
                    /// DANS CE CAS ON UN BUTON A SELECTIONNER
                j=0;
                AffichezButon(buton[j],a+ps[i][j],b+i*pas,10,ColorSelect);
            }else if(i!=4) j=Min(j,saiser[i]); /// ON CHANGE LE VALAUR DE J SI IL DEPASSE LE NOMBRE DE CARACTERE QUI SONT SAISER
                    else j=0;
        }else if (Q&&((c==72)||(c==73))){
            /// IL APPUIYER SUR UP
            ...
            i=(i+Taille-1)%Taille; /// ON DECREMANTE I DE FACON NE DEPASSE PAS LE NOMBRE DE UNPUTE A ENTREZ
            ...
            /// IL APUIYER SUR LEFT
            if (i==Taille-1) {
                AffichezButon(buton[j],a+ps[i][j],b+i*pas,10,ColorDeselect); /// DANS CE CAS ON UN BUTON A DESELECTIONNER j
                j=(j+saiser[i]-1)%(saiser[i]);
                AffichezButon(buton[j],a+ps[i][j],b+i*pas,10,ColorSelect);
            }else j=(j+saiser[i])%(saiser[i]+1); /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            ...
            if (i==Taille-1) {
                ...
                j=(j+saiser[i]+1)%(saiser[i]);
                AffichezButon(buton[j],a+ps[i][j],b+i*pas,10,ColorSelect);
            }else j=(j+1)%(saiser[i]+1); /// ON INCREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            ...
            j=saiser[i]; /// ON DEPELACE LE CURSEUR A LA FIN DE LA PHRASE
             ...
             if ((Q&&(c==83))||(!Q&&(c==8))){
              ...
              if (j>0){
               j--; /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
               Info[i][j]='0';    /// ON REMPLACE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J PAR 0
               cputcxyBlanc(a,b+i*pas,Info[i],saiser[i],&(ps[i][0])); /// ON ECRIT LE CHAINE DANS ECRAN
            }else printf("%c",7); /// ON PEUT PAS SUPRIMER ON INVERTER LE UTILISATAURE AVEC UN SON

          }else if ((!Q)&&(isdigit(c))){
            /// IL C NUMERIQUE
            if (j<saiser[i]){
               Info[i][j]=c; /// ON SAUVERGERDE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
               cputcxyBlanc(a,b+i*pas,Info[i],saiser[i],&(ps[i][0])); /// ON ECRIT LE CHAINE DANS ECRAN
               j++; /// ON INCREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            }else printf("%c",7); /// ON PEUT PAS SAISER ON INVERTER LE UTILISATAURE AVEC UN SON
            }
           ...
           if (j>0){
            j--; /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            SupprimerChar(Info[i],saiser[i],j); /// ON SUPPRIME LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
            cputsxyBlanc(a+j,b+i*pas,&(Info[i][j]),Lm[i]-j); /// ON ECRIT LE CHAINE DANS ECRAN
            saiser[i]--; /// ON DECREMANTE LE NOMBRE DES A SAISER

            }else printf("%c",7); /// ON PEUT PAS SUPPRIMER ON INVERTER LE UTILISATAURE AVEC UN SON

          }else if ((!Q)&&(((i!=3)&&(isalnum(c)||c==32))||((i==3)&&(isdigit(c))))){
            /// SI LE SAISER DE DIFFIRENT DE NUMERO DE TELEPHONE C CARACTERE ALPHAPTIQUE OU NUMERIQUE OU BLANC
            /// SINON LE CARACTERE NUMERIQUE
            if (saiser[i]<Lm[i]){
                if ((c!=32)||((c==32)&&(j>0)&&(Info[i][j-1]!=32)&&(Info[i][j]!=32))){
                        /// ON FAIT CONDITION SI IL YA UN ESPACE BLANC DEJA POUR N'EST PAS LE REPRTER
                    c= (islower(c)) ? c-32 : c ; /// ON TRANFER LES LETTRE EN MAJUSQUILE
                    AjoutezChar(Info[i],saiser[i],j,c); /// ON SAUVERGERDE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
                    cputsxyBlanc(a+j,b+i*pas,&(Info[i][j]),Lm[i]-j); /// ON ECRIT LE CHAINE DANS ECRAN
    ...
    P->NbreDeEmprunt=0;
    P->penaliter=0;
        ///  ON AFFICHE LES INFORMATIONS QUI SONT SAISER DANS ECRAN POUR CONFERMER QUI N'EST PAS ERREUR
    resetcolor();
    system("CLS");
    AffichezCadre(19,1,82,6,35,ColorCadre2);
    cputsxych(22,3,"C'EST LES INFORMATION SONT CORRECT APUIYER SUR ENTRER POR SAUVERGERDER");
    cputsxych(22,4,"SINON APUIYER SUR ECHAP POUR RETOURNER EN ARREIRE POUR REINSCRIT");
...
void AjoutezUnLivre(maillon_Livre**Tete){
   ...
   **/
     ...
     int Taille=15,i,j,Lm[15]={20,40,40,40,8,50,100,100,100,100,100,100,100,100,1};
     int ps[15][110];
     char*  Mot[] = {
                     "COTE ",
                     "TITRE ",
 ...
 debut :                        textbackground(colorbackground);
                system("CLS");
     AffichezCadre(x-1,y-1,lg+2,ht+2,5,colorText);
             ...
             int pas[15];
             for (i=0;i<Taille-1;i++) pas[i]=pa*i;
             for (i=1;i<8;i++) pas[6+i]=pas[6]+i;
...
RST :   /// C'EST ICI LE RETOURE DE GOTO C'EST IL AUIYER SUR LE BUTON RESET
    ...
    for (i=0;i<7;i++){
     ...
     for (i=6;i<14;i++) cputsxyBlanc(a,b+pas[i]," ",Lm[i]);
     ...
     Info[4][8]=0;
     cputcxyBlanc(a,b+pas[4],Info[4],8,&(ps[4][0])); /// ON ECRIT LE CHAINE DE CARACTERE
     int saiser[15]={0};
     saiser[4]=8;
     saiser[Taille-1]=2;
     ps[Taille-1][0]=0;
     ps[Taille-1][1]=20;
     for (j=0;j<2;j++) AffichezButon(buton[j],a+ps[Taille-1][j],b+pas[Taille-1],10,ColorDeselect);/// AFFICHEZ DEUX BUTTON A L'ECRAN
     int Q=false;
     ...
     do {
        ...
        if ((i==Taille-1)&&(!Q)&&(c==13)){
            ...
            if (j==0) {
                ...
                for (k=0;k<7;k++){
                 ...
                 if ((Years<1900)||(Years>2016)) {
                    ...
                    AffichezErreur(x+20,y+1," LA DATE YEARS IL DOIT ENTRE 1900 .. 2016 ");
                    continue;
                 }
                 sprintf(ya,"%c%c",Info[4][2],Info[4][3]);
                 Month=strtol(ya,NULL,10);
                 if ((Month<1)||(Month>12)) {
                        /// SI LE DATE MONTH INCORECT ON SIGNALE UN ERREUR
                    AffichezErreur(x+20,y+1," LA DATE MONTHE EST INCORECT IL DOIT ENTRE 1 .. 12 ");
                    continue;
                 }
                 sprintf(ya,"%c%c",Info[4][0],Info[4][1]);
                 Days=strtol(ya,NULL,10);
                 ...
                 sprintf(resum,"%s %s %s %s %s %s",Info[6],Info[7],Info[8],Info[9],Info[10],Info[11],Info[12],Info[13]);
                    if (NbreMot(resum)>100){
                    /// SI LE NOMBRE DES MOT DE RESUME DEPPASSE 100
                    AffichezErreur(x+20,y+1," LE NOMBRE DES MOT DE RESUME 100 AU MAXIMUM ");
                    continue;
                 }
                 char* rh[20],sauv[30];
                 ...
                 for (k=0;k<Tai;k++){
                    if (strstrcase(resum,rh[k])==0){
                        ...
                        sprintf(ya," LA MOT %d N'EXISTE PAS DANS LE RESUME IL DOIT TOUS LES MOT CLE EXISTE",k+1);
                        AffichezErreur(x+20,y+1,ya);
                        break;
                        }
                        }
            ...
            /// IL APUIYER SUR LEFT
            if (i==Taille-1) {
                AffichezButon(buton[j],a+ps[i][j],b+pas[i],10,ColorDeselect); /// DANS CE CAS ON UN BUTON A DESELECTIONNER j
                j=(j+saiser[i]-1)%(saiser[i]);
                AffichezButon(buton[j],a+ps[i][j],b+pas[i],10,ColorSelect);
            }else j=(j+saiser[i])%(saiser[i]+1); /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            ...
            j=saiser[i]; /// ON DEPELACE LE CURSEUR A LA FIN DE LA PHRASE
             ...
             if ((Q&&(c==83))||(!Q&&(c==8))){
              ...
              if (j>0){
               j--; /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
               Info[i][j]='0';    /// ON REMPLACE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J PAR 0
               cputcxyBlanc(a,b+pas[i],Info[i],saiser[i],&(ps[i][0])); /// ON ECRIT LE CHAINE DANS ECRAN
            }else printf("%c",7); /// ON PEUT PAS SUPRIMER ON INVERTER LE UTILISATAURE AVEC UN SON
           ...
           if (j>0){
            j--; /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            SupprimerChar(Info[i],saiser[i],j); /// ON SUPPRIME LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
            cputsxyBlanc(a+j,b+pas[i],&(Info[i][j]),Lm[i]-j); /// ON ECRIT LE CHAINE DANS ECRAN
            saiser[i]--; /// ON DECREMANTE LE NOMBRE DES A SAISER
            }else printf("%c",7); /// ON PEUT PAS SUPPRIMER ON INVERTER LE UTILISATAURE AVEC UN SON

          }else if ((!Q)&&((((i==2)||(i==3))&&((isalnum(c))||(c==32)))||((i==0)&&(isdigit(c)))||(((i==1)||(i>4))&&(isprint(c))))){
            if (saiser[i]<Lm[i]){
                if ((c!=32)||((c==32)&&(j>0)&&(Info[i][j-1]!=32)&&(Info[i][j]!=32))){
                        /// ON FAIT CONDITION SI IL YA UN ESPACE BLANC DEJA POUR N'EST PAS LE REPRTER
                    if ((i==2)||(i==3)) {c= (islower(c)) ? c-32 : c ;} /// ON TRANFER LES LETTRE EN MAJUSQUILE
                    AjoutezChar(Info[i],saiser[i],j,c); /// ON SAUVERGERDE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
                    cputsxyBlanc(a+j,b+pas[i],&(Info[i][j]),Lm[i]-j); /// ON ECRIT LE CHAINE DANS ECRAN
                    saiser[i]++; /// ON INCREMANTE LE NOMBRE DES A SAISER
                    j++; /// ON INCREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
    ...
    cputsxych(12,3,"C'EST LES INFORMATION SONT CORRECT APUIYER SUR ENTRER POUR SAUVERGARDER");
...
void EmprunterUnLivre(maillon_Emprunt**Tete,maillon_Livre**Livre,maillon_Emprunteur**Emprunteur){
...
RST :   /// C'EST ICI LE RETOURE DE GOTO C'EST IL AUIYER SUR LE BUTON RESET
    ...
    for (i=0;i<Taille-1;i++){
     ...
     Info[2][8]=0;
     cputcxyBlanc(a,b+pas[2],Info[2],8,&(ps[2][0])); /// ON ECRIT LE CHAINE DE CARACTERE
     int saiser[5]={0};
     saiser[2]=8;
     saiser[Taille-1]=2;
     ps[Taille-1][0]=0;
        ...
        if ((i==Taille-1)&&(!Q)&&(c==13)){
            ...
            if (j==0) {
                ...
                for (k=0;k<Taille-2;k++){
                 ...
                 if ((Years<2000)||(Years>2100)) {
                        /// SI LE DATE YEARS < 2000 OU > 2100 ON SIGNALE UN ERREUR
                    AffichezErreur(x+20,y+1," LA DATE YEARS IL DOIT ENTRE 2000 .. 2100 ");
                    continue;
                 }
                 ...
                 pos2=RechachezUnOrdreInscription_Emprunteur(*Emprunteur,strtol(Info[1],NULL,10));
                 if(!pos1) {
                        AffichezErreur(x+20,y+1," CETTE ORDRE D'INSCRIPTION  N'EXSITE PAS ");
                        continue;
                    }
                 ...
                 if (PEmprunteur->Emprunt){
                        AffichezErreur(x+15,y+1," VOUS AVAIS DEJA EMPRUNTER UN LIVRE ET VOUS N'AVAIS PAS RENDER");
                        continue;
                 }
                 if ((PEmprunteur->penaliter)>0) {
                         sprintf(ya,"VOUS ETE DEJA BLOQUER REVIEN APRES %d JOURS",PEmprunteur->penaliter);
            ...
            if (i==Taille-1) {
                ...
                j=0;
                AffichezButon(buton[j],a+ps[i][j],b+pas[i],10,ColorSelect);
            }else if(i!=2) j=Min(j,saiser[i]); /// ON CHANGE LE VALAUR DE J SI IL DEPASSE LE NOMBRE DE CARACTERE QUI SONT SAISER
                    else j=0;
        }else if (Q&&((c==72)||(c==73))){
            /// IL APPUIYER SUR UP
            if (i==Taille-1) { /// DANS CE CAS ON UN BUTON A DESELECTIONNER
    ...
    cputsxych(22,3,"C'EST LES INFORMATION SONT CORRECT APUIYER SUR ENTRER POUR SAUVERGERDER");
    cputsxych(22,4,"SINON APUIYER SUR ECHAP POUR RETOURNER EN ARREIRE POUR REEMPRUNTER");
...
RST :   /// C'EST ICI LE RETOURE DE GOTO C'EST IL AUIYER SUR LE BUTON RESET
    ...
    for (i=0;i<Taille-1;i++){
     ...
     Info[1][8]=0;
     cputcxyBlanc(a,b+pas[1],Info[1],8,&(ps[1][0])); /// ON ECRIT LE CHAINE DE CARACTERE
     int saiser[5]={0};
     saiser[1]=8;
     saiser[Taille-1]=2;
     ps[Taille-1][0]=0;
     ...
     do {
        ...
        PLivre->Emprunt->penaliter=DiffiranceEntre2Date(PLivre->Emprunt->dateDeRetour.days,PLivre->Emprunt->dateDeRetour.month,PLivre->Emprunt->dateDeRetour.years,Days,Month,Years);
        ...
        system("CLS");
        cputsxych(22,3,"LE EMPRUNT EST TERMINER AVEC SECCES MERCI ...");
        } else   cputsxych(22,3,"IL YA ERREUR DANS LA COTE CETTE N'EST PAS EMPRINTER OU DEJA RENDER OU IL YA ERREUR DANS LES FICHIER ...");
        c=getch();
FG:    resetcolor();
}
void AffichezListeEmprunteur(maillon_Emprunteur* tete){
    ...
    do{
        ...
        textbackground(colorbackground0);
        system("CLS");
        if (P!=NULL) Afficher_Emprunteur(P,20,8,80,50,ColorCadre2);
        getch();
    }while(1);
    resetcolor();
}
...
void AffichezListeEmprunt(maillon_Emprunt* Emprunt){
    ...
    while(P!=NULL){
        sprintf(v,"%s PAR %s %s",P->livreEmprunte->titre,P->Emprunteur->nom,P->Emprunteur->prenom);
        T[m]=malloc(strlen(v)*sizeof(char)+2);
        strcpy(T[m],v);
        m++;
        P=suivant_Emprunt(P);
...
debut :     AffichezPage(x,y,lg,ht,colorbackground);
     ...
     do {
        ...
        if ((!Q)&&(c==13)){
            ...
            /// IL APUIYER SUR LEFT
                j=(j+saiser)%(saiser+1); /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
        }else if (Q&&(c==77)){
            /// IL APUIYER SUR RIGHT
                j=(j+1)%(saiser+1); /// ON INCREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
        }else if (Q&&(c==71)){
            /// IL APUIYER SUR DEBUT DE LINE
            j=0; /// ON DEPELACE LE CURSEUR A LA DEBUT DE LA LINE
        }else if (Q&&(c==79)){
           ...
           if (j>0){
            j--; /// ON DECREMANTE J DE FACON NE DEPASSE PAS LE NOMBRE DE CARACTERE QUI SONT SAISER
            SupprimerChar(Info,saiser,j); /// ON SUPPRIME LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
            cputsxyBlanc(a+j,b,&(Info[j]),Lm-j); /// ON ECRIT LE CHAINE DANS ECRAN
            saiser--; /// ON DECREMANTE LE NOMBRE DES A SAISER
            }else printf("%c",7); /// ON PEUT PAS SUPPRIMER ON INVERTER LE UTILISATAURE AVEC UN SON
           }else if ((!Q)&&(isdigit(c))){
            if (saiser<Lm){
                    AjoutezChar(Info,saiser,j,c); /// ON SAUVERGERDE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
                    cputsxyBlanc(a+j,b,&(Info[j]),Lm-j); /// ON ECRIT LE CHAINE DANS ECRAN
     ...
     do {
        ...
        if ((!Q)&&(c==13)){
           ...
           if (j>0){
            ...
            if (saiser<Lm){
                    if ((c!=32)||((c==32)&&(j>0)&&(Info[j-1]!=32)&&(Info[j]!=32))){
                    c= (islower(c)) ? c-32 : c ; /// ON TRANFER LES LETTRE EN MAJUSQUILE
                    AjoutezChar(Info,saiser,j,c); /// ON SAUVERGERDE LA CARACTERE DANS CHAINE INFO[I] A LA POSTION J
                    cputsxyBlanc(a+j,b,&(Info[j]),Lm-j); /// ON ECRIT LE CHAINE DANS ECRAN
                    saiser++; /// ON INCREMANTE LE NOMBRE DES A SAISER
...
gotoxy(z,w+1);
printf("      %c%c%c%c%c%c%c%c%c      %c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+2);
printf("      %c%c%c%c%c%c%c%c%c      %c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+3);
printf("      %c%c       %c%c    %c%c     %c%c       %c%c    %c%c            %c%c             %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
gotoxy(z,w+4);
printf("      %c%c       %c%c    %c%c     %c%c       %c%c    %c%c            %c%c             %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+5);
printf("      %c%c       %c%c    %c%c     %c%c       %c%c    %c%c            %c%c             %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+6);
printf("      %c%c%c%c%c%c%c%c%c      %c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+7);
printf("      %c%c%c%c%c%c%c%c%c      %c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+8);
...
printf("\n");
textcolor(4);
printf("                                         < GESTIONNAIRE DE BIBLIOTHEQUE BIBESI > |");
textcolor(12);
     printf("                              | V 1.0 ");
   puts("                          ");
    textcolor(0);
...
printf("\n");
     textcolor(5);
   printf("                                        |%c%c           REALISE PAR   --->   CHAKLALA LAHCENE | ISMAIL KHERBACH          %c%c|",a,a,a,a);
   textcolor(0);
printf("\n");
   printf("                                        |%c%c___________________________________________________________________________ %c%c|",a,a,a,a);
   textcolor(0);
printf("\n");
   printf("                                        |%c%C                                                                            %c%c|",a,a,a,a);
   textcolor(5);
printf("\n");
   printf("                                        |%c%c           SECTION C    --|-->    1CPI   --|-->    GO9                      %c%c|",a,a,a,a);
   textcolor(5);
printf("\n");
   textcolor(5);
...
printf("\n");
   ...
   textcolor(5);
printf("\n");
   printf("                                        |%c%c           ENCADRE PAR MADAME DAOUDI ASMAA                                  %c%c|",a,a,a,a);
   textcolor(5);
printf("\n");
   textcolor(5);
   printf("                                        |%c%c____________________________________________________________________________%c%c|",a,a,a,a);
...
printf("\n");
   textcolor(0);
   puts("                          ");
printf("                                                       %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
printf("\n");
   textcolor(4);
printf("                                                       %c%c%c%c%c                                          %c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
printf("\n");
printf("                                                       %c%c%c%c%c        | CLIQUER POUR CONTINUER |        %c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
printf("\n");
printf("                                                       %c%c%c%c%c                                          %c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
   textcolor(0);
printf("\n");
...
puts("    ");
textcolor(BLACK);
puts("                                         LES SAUVEGARDEMNT EST TERMINREE AVEC SUCCES .............. ");
gotoxy(10,40);

puts("                                        {LAHCENE CHAKLALA </> ISMAIL KHERBACH ALL COPYRIGHTS RESERVED 2017             ");
textcolor(0);
getch();
textbackground(colorbackground0);
textcolor(colorText0);
...
system("cls");
textcolor(4);
puts("                             ECOLE SUPERIEURE D'INFORMATIQUE ESI {LAHCENE CHAKLALA </> ISMAIL KHERBACH}");
gotoxy(z,w+1);
textcolor(5);
printf("      %c%c%c%c%c%c%c%c%c       %c%c        %c%c      %c%c      %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+2);
printf("      %c%c%c%c%c%c%c%c%c       %c%c        %c%c      %c%c      %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+3);
printf("      %c%c              %c%c        %c%c      %c%c      %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+4);
printf("      %c%c              %c%c        %c%c      %c%c      %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+5);
printf("      %c%c              %c%c        %c%c      %c%c      %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+6);
printf("      %c%c              %c%c        %c%c      %c%c      %c%c      %c%c     %c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+7);
printf("      %c%c   %c%c%c%c       %c%c        %c%c      %c%c      %c%c      %c%c     %c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+8);
printf("      %c%c       %c%c     %c%c        %c%c      %c%c      %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+9);
printf("      %c%c       %c%c     %c%c        %c%c      %c%c      %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+10);
printf("      %c%c       %c%c     %c%c        %c%c      %c%c      %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+11);
printf("      %c%c%c%c%c%c%c%c%c       %c%c%c%c%c%c%c%c%c%c%c%c      %c%c      %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+12);
printf("      %c%c%c%c%c%c%c%c%c       %c%c%c%c%c%c%c%c%c%c%c%c      %c%c      %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a); gotoxy(z,w+13);
printf("\n");
printf("\n");
textcolor(4);
...
puts("                      ");
puts("                      DANS CETTE PAGE VOUS TROUVEZ LE GUIDE POUR BIEN MANUPILER L'APPLICATION");
puts("                      __________________________________________________________________________________");
printf("\n");
printf("\n");
textcolor(0);
...
printf("\n");
printf("         %c%c ---> %c%c     MOVE RIGHT     %c%c  UP  %c%c      MOVE UP",a,a,a,a,a);
printf("\n");
printf("         %c%c      %c%c                    %c%c      %c%c",a,a,a,a,a);
printf("\n");
printf("         %c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
...
printf("         %c%c      %c%c                    %c%c      %c%c",a,a,a,a,a);
printf("\n");
printf("         %c%c <--- %c%c     MOVE LEFT      %c%c DOWN %c%c      MOVE DOWN",a,a,a,a,a);
printf("\n");
printf("         %c%c      %c%c                    %c%c      %c%c",a,a,a,a,a);
printf("\n");
printf("         %c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
...
printf("\n");
printf("         %c%c ESC  %c%c     MOVE BACK      %c%c   <---    %c%c      CLEAR         %c%c SUPPR  %c%c     DELETE TEXT      ",a,a,a,a,a);
printf("\n");
printf("         %c%c      %c%c                    %c%c           %c%c                    %c%c        %c%c                    ",a,a,a,a,a);
printf("\n");
printf("         %c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
printf("\n");
printf("\n");
printf("         %c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
printf("\n");
printf("         %c%c      %c%c                    %c%c      %c%c                    %c%c                %c%c",a,a,a,a,a);
printf("\n");
printf("         %c%c    | %c%c                    %c%c FIN  %c%c   GO TO END PAGE   %c%c     ESPACE     %c%c    SEPARE BETWEEN WORDS EXT ..",a,a,a,a,a);

printf("\n");
printf("         %c%c <--| %c%c  ACCESS TO PAGE    %c%c      %c%c                    %c%c                %c%c",a,a,a,a,a);
printf("\n");
printf("           %c%c    %c%c   ENTER VALUE      %c%c%c%c%c%c%c%c%c%c                    %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",a,a,a,a,a);
printf("\n");
printf("           %c%c    %c%c",a,a,a,a,a);
printf("\n");
printf("           %c%c%c%c%c%c%c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
printf("\n");
puts("                                                                                          ALL COPYRIGHTS RESERVED 2017");
