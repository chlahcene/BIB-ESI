  ...
  while (fgets(ch, 10000, F) != NULL) {
    ...
    if (*Livre == NULL)
      *Livre = P;
      ...
      aff_adr_Livre(Q, P);
    sscanf(ch, "%d %s %s %d/%d/%d %s", &(P->cote), P->titre, P->auteur,
           &(P->dateEdition.days), &(P->dateEdition.month),
           &(P->dateEdition.years), P->editeur);
...
void LireEmprunteur(maillon_Emprunteur **Emprunteur) {
  ...
  *Emprunteur = NULL;
  ...
  while (fgets(ch, 2000, F) != NULL) {
    ...
    if (*Emprunteur == NULL)
      *Emprunteur = P;
      ...
      aff_adr_Emprunteur(Q, P);
    sscanf(ch, "%s %s %s %s %d %d/%d/%d", P->nom, P->prenom, P->adresseActuelle,
           P->nDeTelephone, &(P->ordreInscription),
           &(P->dateDeInscription.days), &(P->dateDeInscription.month),
  ...
  if (P != NULL)
    aff_adr_Emprunteur(P, NULL);
  fclose(F);
}
void Chargement(maillon_Emprunt **Emprunt, maillon_Livre **Livre,
                maillon_Emprunteur **Emprunteur) {

  /// CHARGER TOUS LES INFORMATION QUI STOUQUER DANS LES FICHIER ET LE
  /// SAUVERGERDER DANS DES LISTE
  ...
  while (fgets(ch1, 500, F1) != NULL) {
    ...
    if (*Emprunt == NULL)
      *Emprunt = P;
      ...
      aff_adr_Emprunt(Q, P);
    sscanf(ch1, "%d %d %d/%d/%d %d/%d/%d %d %d", &cote, &ordre,
           &(P->dateDeEmprunt.days), &(P->dateDeEmprunt.month),
           &(P->dateDeEmprunt.years), &(P->dateDeRetour.days),
           &(P->dateDeRetour.month), &(P->dateDeRetour.years), &(P->livreRendu),
           &(P->penaliter));
    i = RechachezUnCote_Livre(*Livre, cote);
    y = P->dateDeRetour.years;
    d = P->dateDeRetour.days;
    m = P->dateDeRetour.month;
    if (P->penaliter > 0) {
...
void EcrireLivre(maillon_Livre **Livre) {
  ...
  while (P != NULL) {
    ...
    fprintf(F, "%d %s %s %d/%d/%d %s (%s) #%s#\n", P->cote, P->titre, P->auteur,
            P->dateEdition.days, P->dateEdition.month, P->dateEdition.years,
            P->editeur, P->motCle, P->resume);
    P = suivant_Livre(P);
  }
  fclose(F);
}
void EcrireEmprunteur(maillon_Emprunteur **Emprunteur) {
  ...
  /// ECRIRE DANS FICHIER EMPRUNTEUR LES INFORMATION QUI SONT DANS LISTE
  /// EMPRUNTEUR
  ...
  while (P != NULL) {
    ...
    fprintf(F, "%s %s %s %s %d %d/%d/%d\n", P->nom, P->prenom,
            P->adresseActuelle, P->nDeTelephone, P->ordreInscription,
            P->dateDeInscription.days, P->dateDeInscription.month,
            P->dateDeInscription.years);
...
void EcrireEmprunt(maillon_Emprunt **Emprunt) {
  ...
  /// ECRIRE DANS FICHIER EMPRUNT ET OTHERS LES INFORMATION QUI SONT DANS LISTE
  /// EMPRUNT
  ...
  P = *Emprunt;
  while (P != NULL) {
    fprintf(F1, "%d %d %d/%d/%d %d/%d/%d %d %d\n", P->livreEmprunte->cote,
            P->Emprunteur->ordreInscription, P->dateDeEmprunt.days,
            P->dateDeEmprunt.month, P->dateDeEmprunt.years,
            P->dateDeRetour.days, P->dateDeRetour.month, P->dateDeRetour.years,
            P->livreRendu, P->penaliter);
    P = suivant_Emprunt(P);
  }
  fclose(F1);
}
void Sauvergarder(maillon_Emprunt **Emprunt, maillon_Livre **Livre,
                  maillon_Emprunteur **Emprunteur) {
...
void EclaterEmprunt(maillon_Emprunt *Emprunt) {
  ...
  cputsxych(12, 4,
            "ALLER VOIR LE FICHIER Fiche Emprunt En cours.txt ET Fiche Emprunt "
            "En cours.txt ...");
  getch();
}
...
int RechachezUnAdresseActuelle_Emprunteur(maillon_Emprunteur *liste,
...
int RechachezUnOrdreInscription_Emprunteur(maillon_Emprunteur *liste,
...
void Eclater2ListeEmprunt(maillon_Emprunt *emprunt,
                          maillon_Emprunt **empruntEncours,
                          maillon_Emprunt **empruntNonEncours) {
  ...
  *empruntNonEncours = NULL;
  while (P != NULL) {
    ...
    (*Q) = (*P);
    if (P->livreRendu) {
      if (*empruntNonEncours == NULL)
        *empruntNonEncours = Q;
      else
        aff_adr_Emprunt(P1, Q);
      P1 = Q;
    } else {
      if (*empruntEncours == NULL)
        *empruntEncours = Q;
      else
        aff_adr_Emprunt(P2, Q);
      P2 = Q;
    }
    P = suivant_Emprunt(P);
  }
  if (P1 != NULL)
    aff_adr_Emprunt(P1, NULL);
  if (P2 != NULL)
    aff_adr_Emprunt(P2, NULL);
}
...
void RechacheParMotsCles(maillon_Livre *Livre, char mot[],
                         maillon_Livre *tab[]) {
  ...
  if (tab[0] != NULL)
    printf("%s", tab[0]->auteur);
  (tab)[i] = NULL;
}
void RechacheParAuteur(maillon_Livre *Livre, char Auteur[],
                       maillon_Livre *tab[]) {
...
void Eclater2ListeLivres(maillon_Livre *Livre, maillon_Livre **LivreDisponible,
                         maillon_Livre **LivreNonDisponible) {
  ...
  *LivreDisponible = NULL;
  while (P != NULL) {
    ...
    if (P->disponible) {
      ...
      if ((*LivreNonDisponible) == NULL)
        (*LivreNonDisponible) = Q;
      else
        aff_adr_Livre(P2, Q);
      P2 = Q;
    }
  ...
  if (P1 != NULL)
    aff_adr_Livre(P1, NULL);
  if (P2 != NULL)
    aff_adr_Livre(P2, NULL);
}
void CalculePinaliterEncours(maillon_Emprunteur **Emprunteur) {
  ...
  while (P != NULL) {
    if ((P->Emprunter) != 0) {
      i = DiffiranceEntre2Date(
          P->Emprunt->dateDeRetour.days, P->Emprunt->dateDeRetour.month,
          P->Emprunt->dateDeRetour.years, Days, Month, Years);
      if (i > 0)
        (P->penaliterTotal) += i;
    }
    P = suivant_Emprunteur(P);
  ...
  if (P == *Emprunteur)
    *Emprunteur = NULL;
  if (P != NULL)
    viderListe_Emprunteur(&P);
  if (Q != NULL)
    aff_adr_Emprunteur(Q, NULL);
}
